  ...
  return (
    <div>
      ...
      <table width="100%" border="1" cellPadding="8" style={{ borderCollapse: "collapse" }}>
        <thead><tr><th>ID</th><th>Hora início</th><th>Hora fim</th><th>Capacidade</th><th>Ocupada</th><th>Status</th></tr></thead>
        <tbody>
          {data.janelasHoje.map((j) => (
            <tr key={j.id}>
              <td>{j.id}</td><td>{j.horaInicio}</td><td>{j.horaFim}</td>
              <td>{j.capacidadeMaxima}</td><td>{j.capacidadeOcupada}</td><td>{j.status}</td>
            </tr>
          ))}
        </tbody>
      </table>
