(() => {
  ...
  function noteEmpresaMatches(nota, target) {
    const empresa = normalizeCompanyKey(nota?.empresa || nota?.empresaRelatorio || '');
    const desired = normalizeCompanyKey(target);
    if (!empresa || !desired) return false;
    return empresa === desired || empresa.includes(desired) || desired.includes(empresa);
  }
  ...
  function renderPendingNotasInterno() {
    ...
    wrap.innerHTML = `<div class="pending-notas-toolbar"><button type="button" class="btn-secondary" id="btnSelectAllPendingNotas">Selecionar todos</button></div>${groups.map((group) => `<div class="pending-notas-group${group.highlight ? ' pending-notas-group-highlight' : ''}"><h4>${escapeHtml(group.title)} <span>${escapeHtml(formatIntegerBR(group.items.length))} NF</span></h4><div class="pending-notas-grid">${group.items.map((nota, idx) => {
      ...
      return `<div class="pending-nota-item${dueClass}" title="${escapeHtml(tooltip)}"><label class="pending-nota-card"><div class="pending-nota-check"><input type="checkbox" data-internal-nf="${originalIndex}" /><span>${escapeHtml(label)}</span>${empresa}${dueBadge}</div><div class="pending-nota-meta"><span><strong>Entrada:</strong> ${escapeHtml(dataEntrada)}</span><span><strong>Peso:</strong> ${escapeHtml(formatDecimalBR(nota.peso || 0, 3))} kg</span><span><strong>Volumes:</strong> ${escapeHtml(formatDecimalBR(nota.volumes || 0, 3))}</span></div></label></div>`;
    }).join('')}</div></div>`).join('')}`;
  ...
  async function loadFornecedoresPendentesInterno() {
    try {
      ...
      select.innerHTML = `<option value="">Selecione o fornecedor pendente</option>` + state.pendingFornecedores.map((item) => {
        const highlight = item.possuiVencimentoProximo ? ` • ${formatIntegerBR(item.totalNotasVencimentoProximo || 0)} NF com vencimento próximo` : '';
        return `<option value="${escapeHtml(item.id || '')}">${escapeHtml(item.fornecedor || item.nome || '-')} (${escapeHtml(item.quantidadeNotas ?? 0)} NF${escapeHtml(highlight)})</option>`;
      }).join('');
      select.onchange = () => {
        if (!select.value) {
  ...
  function renderConsultaNfResult(data = {}) {
    ...
    return `
      <div class="consulta-nf-summary-grid">
      ...
      <div class="consulta-nf-result-grid">
        <div class="result-card">
          ...
          ${relatorio.length ? `<table class="table"><thead><tr><th>Fornecedor</th><th>Empresa</th><th>NF</th><th>Série</th><th>Data entrada</th><th>1º vencimento</th><th>Status</th><th>Agendada?</th></tr></thead><tbody>${relatorio.map((item) => `<tr><td>${escapeHtml(item.fornecedor || '-')}</td><td>${escapeHtml(item.empresa || '-')}</td><td>${escapeHtml(item.numeroNf || '-')}</td><td>${escapeHtml(item.serie || '-')}</td><td>${escapeHtml(item.dataEntradaBr || '-')}</td><td>${escapeHtml(item.dataPrimeiroVencimentoBr || '-')}</td><td>${escapeHtml(item.status || '-')}</td><td>${item.agendamentoId ? 'Sim' : (resumo.agendada ? 'Sim' : 'Não')}</td></tr>`).join('')}</tbody></table>` : '<p class="hint">NF não localizada no relatório terceirizado atual.</p>'}
        </div>
        <div class="result-card">
          <h3>Agendamentos vinculados (${agendamentos.length})</h3>
          ${agendamentos.length ? `<table class="table"><thead><tr><th>ID</th><th>Protocolo</th><th>Status</th><th>Agendada?</th><th>Usuário</th><th>Data agendada</th><th>Faltam</th><th>Hora</th><th>Data entrada</th><th>1º vencimento</th></tr></thead><tbody>${agendamentos.map((item) => {
            const notas = Array.isArray(item.notasFiscais) ? item.notasFiscais : [];
            const notaConsultada = notas.find((nota) => normalizeDigitsValue(nota?.numeroNf || '').includes(targetDigits)) || notas[0] || {};
            ...
            return `<tr><td>${escapeHtml(item.id || '-')}</td><td>${escapeHtml(item.protocolo || '-')}</td><td>${renderStatusBadge(item.status, item.semaforo || '')}</td><td>Sim</td><td>${escapeHtml(item.usuarioAgendamento || '-')}</td><td>${escapeHtml(formatDateBR(item.dataAgendada || ''))}</td><td>${escapeHtml(faltam)}</td><td>${escapeHtml(formatHour(item.horaAgendada || ''))}</td><td>${escapeHtml(notaConsultada?.dataEntradaBr || notaConsultada?.dataEntrada || '-')}</td><td>${escapeHtml(notaConsultada?.dataPrimeiroVencimentoBr || notaConsultada?.dataPrimeiroVencimento || '-')}</td></tr>`;
          }).join('')}</tbody></table>` : '<p class="hint">Nenhum agendamento encontrado para esta NF. Situação atual: não agendada.</p>'}
        </div>
      </div>
    `;
  }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    byId("consultaNfForm")?.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const numeroNf = formData.get('numeroNf');
        const dataAgendada = formData.get('dataAgendada');
