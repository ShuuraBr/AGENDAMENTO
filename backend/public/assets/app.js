(() => {
  ...
  function renderPendingNotasInterno() {
    ...
    const sync = () => {
      state.internalSelectedNotas = notas.filter((_nota, index) => wrap.querySelector(`[data-internal-nf="${index}"]`)?.checked).map((nota) => ({ ...nota }));
      updateInternalTotals();
    ...
    state.internalSelectedNotas = notas.map((nota) => ({ ...nota }));
    updateInternalTotals();
    syncPendingNotasSelectionUI(wrap);
  }
