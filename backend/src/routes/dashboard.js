...
import { readAgendamentos, readDocumentos, buildDocaPainelFromFiles, readAuditLogs } from "../utils/file-store.js";
...
router.get("/logs", async (req, res) => {
  ...
  try {
    ...
    console.error("Erro em /dashboard/logs. Usando fallback em arquivo:", error?.message || error);
    let items = readAuditLogs().map(normalizeLog);
    if (entidade) items = items.filter((item) => String(item.entidade || "") === entidade);
    if (acao) items = items.filter((item) => String(item.acao || "") === acao);
    if (entidadeId != null && Number.isFinite(entidadeId)) items = items.filter((item) => Number(item.entidadeId) === entidadeId);
    return res.json(items.sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))).slice(0, limit));
  }
});
