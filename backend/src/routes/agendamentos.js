...
async function notifyControladoriaAvaria({ agendamento = {}, payload = {}, req, actor = null, files = [] } = {}) {
...
function buildNotasResumo(notas = []) {
  return (Array.isArray(notas) ? notas : [])
    .map((nota) => `NF ${nota?.numeroNf || '-'}${nota?.serie ? ` / Série ${nota.serie}` : ''}`)
    .filter(Boolean);
}
