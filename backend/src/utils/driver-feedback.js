...
async function findAgendamentoForFeedback(record = {}) {
  ...
  if (protocolo) {
    ...
    const fileProtocol = readAgendamentos().find((item) => String(item?.protocolo || '').trim() === protocolo);
  ...
  if (placa && motorista) {
    ...
    return readAgendamentos().find((item) => String(item?.placa || '').trim().toUpperCase() === placa && String(item?.motorista || '').trim() === motorista) || null;
  }
