...
async function ensureRelatorioTable() {
  ...
  if (!indexNames.has('uk_relatorio_row_hash')) {
    await prisma.$executeRawUnsafe(`ALTER TABLE ${quoteIdentifier(TABLE_NAME)} ADD UNIQUE INDEX uk_relatorio_row_hash (rowHash)`);
  }
  if (!indexNames.has('idx_relatorio_agendamento')) {
    await prisma.$executeRawUnsafe(`ALTER TABLE ${quoteIdentifier(TABLE_NAME)} ADD INDEX idx_relatorio_agendamento (agendamentoId)`);
  }
}
