...
export function enrichAgendamentoRecord(item = {}) {
  ...
  return {
    ...item,
    checkinToken: String(item?.checkinToken || item?.tokenCheckin || item?.checkin_token || item?.token_checkin || '').trim(),
    checkoutToken: String(item?.checkoutToken || item?.tokenCheckout || item?.checkout_token || item?.token_checkout || '').trim(),
    publicTokenMotorista: String(item?.publicTokenMotorista || item?.tokenMotorista || item?.public_token_motorista || '').trim(),
    publicTokenFornecedor: String(item?.publicTokenFornecedor || item?.tokenFornecedor || item?.public_token_fornecedor || '').trim(),
...
export function findAgendamentoByTokenFile(token) {
  const normalizedToken = sanitizeTokenValue(token);
  ...
  return readAgendamentos().find((item) => {
    const itemTokens = [item.publicTokenFornecedor, item.publicTokenMotorista, item.checkinToken, item.checkoutToken, item.tokenFornecedor, item.tokenMotorista, item.tokenCheckin, item.tokenCheckout, item.public_token_fornecedor, item.public_token_motorista, item.checkin_token, item.checkout_token]
      .map((value) => sanitizeTokenValue(value))
      .filter(Boolean);
